...
const swup = new Swup({
  plugins: [
    // will put the initial page into the cache automatically
...
function updateCache(containerSelector, items, nextLink) {
  ...
  cachedDocument
    .querySelector(nextLinkSelector)
    ?.replaceWith(nextLink.cloneNode(true));
...
function initInfiniteScroll() {
  ...
  infScroll.on("append", (doc, _path, items) => {
    ...
    const nextLink =
      doc.querySelector(nextLinkSelector) ?? doc.createElement("span");
